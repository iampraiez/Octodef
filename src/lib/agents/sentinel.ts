     ...
     try {
         ...
         if (key) {
             ...
             try {
                ...
                clearTimeout(timeoutId);
                // Silent fail for user/log cleanliness, just push warning finding
                findings.push({
                     agent: "Sentinel",
                     type: "warning",
                     message: "GSB Lookup Skipped",
                     details: "Connection to Google Safe Browsing timed out."
                });
             }
         }
     } catch (e) {
               ...
               try {
                ...
                if (data.data && data.data.abuseConfidenceScore > 0) {
                   ...
                   findings.push({
                        agent: "Sentinel",
                        type: "warning",
                        message: "IP Lookup Skipped",
                        details: "Connection to AbuseIPDB timed out."
                   });
               }
          }
      } catch (e) {
